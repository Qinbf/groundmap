...
- **source_count 一致性问题（`source_issues_count`，六类：count-mismatch / missing-source / analysis-undersourced / source-summary-mismatch / broken-source-link / declared-but-uncited — 详见 AGENTS.md "source_count 字段约定" 与本文第 6d 步）**
- **关系类型问题（`relation_issues_count`，`[[X|RELATION]]` 中非标准关系类型词——拼写错误 / 未在白名单，详见 AGENTS.md "关系类型语法"）**
- **Web i18n 违规（`i18n_violations_count`，web/ 下硬编码中文 UI 字符串，违反 AGENTS.md "Web 管理台国际化方案"）**
...
如果发现不一致：
- 手写 `> [!WARNING] 知识更新冲突` 块（详见 AGENTS.md "冲突处理规范"），不要默默修正
- 在周报中列出
...
扫描 `web/` 下 `.tsx` 文件，找硬编码的中文 UI 字符串（`<button>中文</button>` / `placeholder="中文"` / `aria-label="中文"` 等）。AGENTS.md "Web 管理台国际化方案" 明令所有 UI 字符串走 `t()` / `useT()`，硬编码会让英文用户看不懂。
...
扫描 `[[X|RELATION]]` 三段链接中**看起来像关系类型、但不在白名单**的词（对应 health 的 `relation_issues_count`；典型是拼写错误 `SUPORTS` 或用了非标准词 `IMPLEMENTS`）。白名单 7 个：`SUPPORTS` / `REFUTES` / `EXTENDS` / `IS_A` / `PART_OF` / `ALTERNATIVE_TO` / `CITES`（详见 AGENTS.md "关系类型语法"）。
