 ...
 * 不渲染，主区独占。
 *
 * 状态持久化：cookie，server 第一帧就拿到，避免首屏 "展开 → 折叠" 闪烁。
 *
 * Auto-collapse 规则（关键）：
  ...
  return (
    <>
      ...
      {/* 按钮 fixed 在视口内（不属于 aside 的 scrollWidth，避免按钮突出
          导致页面级横滚）。
          位置：right-80 + -translate-x-full + 一点点偏移 → 按钮整体在主区
          一侧，右边线紧贴右栏左边线。这样背景色不会从按钮中心切换，避免
          视觉"骑墙"漂移感。*/}
      <button
        type="button"
        onClick={toggle}
        ...
        className={cn(
          baseButtonClass,
          "fixed right-80 top-1/2 z-30 -translate-y-1/2 bg-background/90 backdrop-blur-sm",
        )}
      >
