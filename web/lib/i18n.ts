...
export const TRANSLATIONS = {
  zh: {
    ...
    "walkthrough.thought.view_index": "领域全景 · 查询的硬前置（v0.6 起的硬必读）。AI 先 `read-section` 取 `wiki/root_index.md` 的 `## 领域目录` 节，拿到 N 个领域子索引的全景（结构化目录）。BM25 字面匹配会漏掉跨领域入口，root_index 是结构化兜底网——即使后续要走 BM25 一击命中，也必须先看到领域全景再决策。",
    "walkthrough.thought.strategy": "路径决策 · 看领域全景后才做的选最短路径。基于上一步拿到的 N 个领域入口，按关键词独特性在 (A) 钻 MOC vs (B) 直接 BM25 之间选一条；术语越独特越倾向 (B)。即使走 (B)，也带着领域全景上下文、避免在单领域查得过深漏掉相邻入口。",
    ...
    "learn.step.3.title": "AI 综合判断（基于 wiki 现状 · 并发 5 层）",
    "learn.step.3.why": "AI 不孤立地总结原文，而是按 5 层召回兜底机制**并发**获取 wiki 现状、然后综合三件事（核心价值 / 关联 / 冲突）。v0.6 起新增 ① 层为硬前置：`read-section wiki/root_index.md \"## 领域目录\"` 拿领域全景（结构化目录 BM25 召回不到），其它 ② 多 query / ⑤ 多 read-section 都天然并发（不阻塞）。这些判断会写入摘要页的「AI 综合判断」节，便于你日后审计或在 web 端覆盖。",
    ...
    "learn.caption.root_index_hard_prereq": "v0.6 新增的硬前置 · ① 层 —— `read-section wiki/root_index.md \"## 领域目录\"` 拿到的 8 行结构化目录。BM25 字面召回拿不到这种「分类索引」，硬必读一次后 agent 才知道这篇论文落在哪 1-3 个领域入口（这里是 RAG × memory × open-domain QA 三者交集）。",
    "learn.caption.recall_mechanism": "机制说明，不是文件——v0.6 起升级为 5 层机制：① 硬前置 root_index（新增）/ ② 多 query 并发 / ③ backlinks·outlinks / ④ MOC 结构性回链 / ⑤ 多 read-section 并发（agent 二次判断）。",
  ...
  en: {
    ...
    "walkthrough.thought.view_index": "Domain panorama · a hard prerequisite since v0.6. The AI first calls `read-section` on `wiki/root_index.md`'s `## 领域目录` section to get an N-domain overview (a structured catalog). BM25 is literal-matching and misses cross-domain entries; root_index is a structural fallback net — even when the next step will be a one-shot BM25 search, the AI must see the domain panorama first.",
    "walkthrough.thought.strategy": "Path decision · choose the shortest path AFTER seeing the domain panorama. Given the N domain entries from the previous step, the AI picks between (A) drill into a MOC and (B) direct BM25 by keyword distinctiveness — the more distinctive, the more it leans toward (B). Even on (B) the panorama is the context, so it never tunnels too deep into one domain and misses adjacent entries.",
    ...
    "learn.step.3.title": "AI synthesizes (in light of existing wiki · 5 concurrent layers)",
    "learn.step.3.why": "AI doesn't summarize in a vacuum. It applies a 5-layer recall safety net **concurrently** to capture the wiki's current state, then synthesizes three things (core value / relations / conflicts). Since v0.6 layer ① is a hard prerequisite: `read-section wiki/root_index.md \"## 领域目录\"` for the domain panorama (a structured index BM25 cannot reach); the other layers — ② multi-query and ⑤ multi-read-section — are naturally concurrent (no blocking). These judgments land in the summary page's 'AI synthesis' section for later audit or web-console override.",
    ...
    "learn.caption.root_index_hard_prereq": "v0.6's new hard prerequisite · layer ① — the 8-row structured catalog returned by `read-section wiki/root_index.md \"## 领域目录\"`. BM25 literal recall cannot reach a 'classification index' like this; after the hard read the agent knows which 1–3 domain entries this paper falls under (here: the intersection of RAG × memory × open-domain QA).",
    "learn.caption.recall_mechanism": "Mechanism explainer, not a file — since v0.6 upgraded to 5 layers: ① hard-prerequisite root_index (new) / ② concurrent multi-query / ③ backlinks·outlinks / ④ MOC structural back-links / ⑤ concurrent multi-read-section (agent's second-pass judgment).",
