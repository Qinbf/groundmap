...
- **并行批量读取（提速关键）**：当你已经确定要读的**多个页面彼此独立**（先读哪个都行、一个页面的内容不会改变你要不要读另一个）时，**在同一步里一次性发出多个 \`read_page\` / \`read_section\` 调用**——控制台会并行执行、一次往返就拿回全部结果，省掉逐个等待的多轮往返。典型场景：search 返回了 2-3 个都该看的高相关 hit、或你要同时对照几个已知页面。用**一个 \`**【READ】**\` 步**统一说明「我要并行读这几页、各自想确认什么」即可，不必拆成多步多轮。
- ANSWER 简洁直接，附核心引用即可

**禁忌**：
- 不要 BFS 遍历所有 outlinks
- 不要为了"全面"读 5+ 个页面——超出预算 = 失败
- **不要把"本该串行"的读取硬凑成并行**：若你必须先看到 A 的内容（或 A 的 outlinks / 某次 search 的结果）才能判断要不要读 B，就老实分步读——盲猜 B 读错页面，比多一次往返更伤答案与引用质量。并行只用于"已经确定、且互不依赖"的那批读取。
`;
